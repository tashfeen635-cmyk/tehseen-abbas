...
const DEFAULT_PASSWORD_HASH_KEY = "adminPasswordHash";

export function getAdminHash() {
  const hash = getSetting("adminPasswordHash");
  return hash;
}
...
export function setAdminPassword(plain) {
  ...
  setSetting("adminUsername", process.env.ADMIN_USER || "admin");
}
...
export function verifyAdmin(username, password) {
  ...
  const storedName = getSetting("adminUsername") || "admin";
